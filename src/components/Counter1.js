...
class Counter extends React.Component {
  render() {
    const { number, add, minus, promiseAdd, thunkMinus } = this.props;
    return (
      <React.Fragment>
        <div>{number}</div>
        <input onClick={add} type="button" value="+" />
        <input onClick={minus} type="button" value="-" />
        <input onClick={promiseAdd} type="button" value="promiseAdd +" />
        <input onClick={thunkMinus} type="button" value="thunkMinus -" />
      </React.Fragment>
    );
  }
}
